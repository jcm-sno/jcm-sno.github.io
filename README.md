# James & Samantha Wedding Website

This repository is the public source mirror for the James & Samantha wedding
website. The same verified source snapshot is deployed to both OpenAI Sites and
GitHub Pages.

## Deployment

- `npm run build` produces the OpenAI Sites/Cloudflare Worker artifact.
- `npm run build:pages` produces a checked static export in `dist/client` for
  `https://jcm-sno.github.io`.
- `.github/workflows/deploy-pages.yml` rebuilds and deploys GitHub Pages after
  every push to `main`; it can also be run manually from GitHub Actions.

OpenAI Sites keeps its own source remote. When editing from its managed
checkout, preserve that remote and mirror the verified source to GitHub rather
than replacing or force-pushing the Sites source history.
